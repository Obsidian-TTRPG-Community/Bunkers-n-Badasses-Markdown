...
When you Move, your next Attack gains `$=const slevel = dv.current().slevel; dv.span(slevel);` Damage.

+`$=const slevel = dv.current().slevel; dv.span(slevel);` to Traverse Checks.
